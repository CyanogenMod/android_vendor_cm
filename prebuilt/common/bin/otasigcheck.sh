...
if [ -f /data/system/packages.xml -a -f /tmp/releasekey ]; then
  mmsCert=$(grep -A3 'package name="com.android.mms"' /data/system/packages.xml  | grep "cert index" | head -n 1 | sed -e 's|.*"\([[:digit:]]\)".*|\1|g')

  grep "cert index=\"$mmsCert\"" /data/system/packages.xml | grep -q `cat /tmp/releasekey`
  if [ $? -ne 0 ]; then
