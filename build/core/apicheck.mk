...
# INTERNAL_PLATFORM_CM_API_FILE is the one build by droiddoc.
# Note that since INTERNAL_PLATFORM_CM_API_FILE  is the byproduct of api-stubs module,
...
$(eval $(call check-api, \
    checkpublicapi-last, \
    $(CM_SRC_API_DIR)/$(last_released_sdk_version).txt, \
    $(INTERNAL_PLATFORM_CM_API_FILE), \
...
$(eval $(call check-api, \
    ...
    $(INTERNAL_PLATFORM_CM_REMOVED_API_FILE), \
...
update-cm-public-api: $(INTERNAL_PLATFORM_CM_API_FILE) | $(ACP)
	@echo -e ${CL_GRN}"Copying cm_current.txt"${CL_RST}
	$(hide) $(ACP) $(INTERNAL_PLATFORM_CM_API_FILE) $(TOPDIR)vendor/cmsdk/api/cm_current.txt
	@echo -e ${CL_GRN}"Copying cm_removed.txt"${CL_RST}
	$(hide) $(ACP) $(INTERNAL_PLATFORM_CM_REMOVED_API_FILE) $(TOPDIR)vendor/cmsdk/api/cm_removed.txt

$(info test $(INTERNAL_PLATFORM_CM_API_FILE))
...
$(eval $(call check-api, \
    checksystemapi-last, \
    $(CM_SRC_SYSTEM_API_DIR)/$(last_released_sdk_version).txt, \
    $(INTERNAL_PLATFORM_CM_SYSTEM_API_FILE), \
    vendor/cmsdk/api/cm_system-removed.txt, \
    $(INTERNAL_PLATFORM_CM_SYSTEM_REMOVED_API_FILE), \
    cat $(BUILD_SYSTEM)/apicheck_msg_last.txt, \
    check-cm-system-api, \
...
$(eval $(call check-api, \
    ...
    vendor/cmsdk/api/system-current.txt, \
    $(INTERNAL_PLATFORM_SYSTEM_API_FILE), \
    vendor/cmsdk/api/system-removed.txt, \
...
update-cm-system-api: $(INTERNAL_PLATFORM_CM_SYSTEM_API_FILE) | $(ACP)
	...
	$(hide) $(ACP) $(INTERNAL_PLATFORM_CM_SYSTEM_API_FILE) vendor/cmsdk/api/cm_system-current.txt
	@echo Copying system-removed.txt
	$(hide) $(ACP) $(INTERNAL_PLATFORM_CM_SYSTEM_REMOVED_API_FILE) vendor/cmsdk/api/cm_system-removed.txt
