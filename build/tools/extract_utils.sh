...
# $4: verbose adb pull
#
# Silently extracts the input file to defined target ( if not specified )
...
function oat2dex() {
    ...
    if [ ! -f "$TMPDIR/boot_arm64.oat" ] && [ ! -f "$TMPDIR/boot_arm.oat" ]; then
        if get_file "system/framework/arm64/boot.oat" "$TMPDIR/boot_arm64.oat" "$SRC" "true"; then
            ARCHES+="arm64 "
        fi

        if get_file "system/framework/arm/boot.oat" "$TMPDIR/boot_arm.oat" "$SRC" "true"; then
            ARCHES+="arm "
        fi
