...
function oat2dex() {
    ...
    if [ ! -f "$TMPDIR/boot.oat" ]; then
        if [ "$SRC" = "adb" ]; then
            _adb_silent_pull "/system/framework/arm64/boot.oat" "$TMPDIR"
            if [ "$?" == "0" ]; then
                export ARCH="arm64"
            else
                _adb_silent_pull "/system/framework/arm/boot.oat" "$TMPDIR"
                if [ "$?" == "0" ]; then
                    export ARCH="arm"
                else
    ...
    if [ "$SRC" = "adb" ]; then
        _adb_silent_pull "$D_OAT" "$TMPDIR"
        if [ "$?" == "0" ]; then
            OAT="$TMPDIR/`basename $D_OAT`"
        else
            _adb_silent_pull "$O_OAT" "$TMPDIR"
            if [ "$?" == "0" ]; then
