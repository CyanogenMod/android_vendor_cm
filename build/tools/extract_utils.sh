...
# $4: force verbose adb pull
#
# Silently extracts the input file to defined target, unless verbose output is specified
...
function oat2dex() {
    ...
    for ARCH in "arm64" "arm" "x86_64" "x86"; do
        get_file "system/framework/$ARCH/boot.oat" "$TMPDIR/boot_$ARCH.oat" "$SRC" "true"
        if [ -f "$TMPDIR/boot_$ARCH.oat" ]; then
            ARCHES+="$ARCH "
        fi
    done
    ...
    for ARCH in $ARCHES; do
        ...
        local TMP_OAT="$TMPDIR/`basename $OAT`"

        get_file "$OAT" "$TMPDIR" "$SRC"
        if [ -f "$TMP_OAT" ]; then
            java -jar "$BAKSMALIJAR" -x -o "$TMPDIR/dexout" -c $BOOTOAT -d "$TMPDIR" "$TMP_OAT"
        ...
        if [ "$?" == "0" ]; then
            ...
            if [[ "$FULLY_DEODEXED" -ne "1" && "$DEST" =~ .(apk|jar)$ ]]; then
                ...
                if [ -f "$TMPDIR/classes.dex" ]; then
                    ...
                    echo "    (updated "/$FILE" from odex files)"
                fi
            fi
        fi
