...
if [ ! -d "$ANDROID_BUILD_TOP" ]; then
    echo "\$ANDROID_BUILD_TOP must be set before including this script!"
    exit 1
fi

OUTDIR=vendor/$VENDOR/$DEVICE
export PRODUCTMK=$ANDROID_BUILD_TOP/$OUTDIR/$DEVICE-vendor.mk
export ANDROIDMK=$ANDROID_BUILD_TOP/$OUTDIR/Android.mk
export BOARDMK=$ANDROID_BUILD_TOP/$OUTDIR/BoardConfigVendor.mk
PACKAGE_LIST=()
