...
BOOTANIMATION_CHECK := $(shell sed 's/^vertical-\(.*\)x.*$$/\1/' <<< "$(TARGET_BOOTANIMATION_NAME)")
ifneq ($(TARGET_BOOTANIMATION_NAME),$(BOOTANIMATION_CHECK))
    PRODUCT_COPY_FILES += \
        vendor/cm/prebuilt/common/bootanimation/$(BOOTANIMATION_CHECK).zip:system/media/bootanimation.zip
else
BOOTANIMATION_CHECK := $(shell sed 's/^horizontal-.*x\(.*\)$$/\1/' <<< "$(TARGET_BOOTANIMATION_NAME)")
ifneq ($(TARGET_BOOTANIMATION_NAME),$(BOOTANIMATION_CHECK))
    PRODUCT_COPY_FILES += \
        vendor/cm/prebuilt/common/bootanimation/$(BOOTANIMATION_CHECK).zip:system/media/bootanimation.zip
else
